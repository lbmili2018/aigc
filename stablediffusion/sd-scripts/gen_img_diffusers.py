        ...
        **kwargs,
        ...
        if init_image is None:
            ...
            if init_image.size()[-2:] == (height // 8, width // 8):
                ...
                print("self.init_latent:", init_latents)
            ...
            print("Diffusion Process timesteps:", timesteps)
            latents = self.scheduler.add_noise(init_latents, img2img_noise, timesteps)
            # latents = torch.load("/root/limiao/result/res_universe/samples_ddim.pt").to(torch.float16).to("cuda")
            print("Diffusion Process lanten Z :", latents)
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            # print("N Unet noise_pred:", noise_pred.shape)
        ...
        if vae_batch_size >= batch_size:
            print("final_noise_pred to latent:", latents)
            image = self.vae.decode(latents).sample
            print("latent to decoder_img:", image)
        else:
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            images = []
        ...
        print("decoder_img to gen_img:", image)
...
def preprocess_image(image):
    ...
    image = torch.from_numpy(image)
    print("2.0 * image - 1.0:", 2.0 * image - 1.0)
...
def main(args):
    ...
    if init_images is not None and len(prompt_list) == 0 and not args.interactive:
        ...
        len("len_init_images111:", len(init_images))

        l = []
        for im in init_images:
            l.extend([im] * args.images_per_prompt)
        init_images = l
        len("len_init_images222:", len(init_images))
        ...
        def process_batch(batch: List[BatchData], highres_fix, highres_1st=False):
            ...
            for i, (_, (_, prompt, negative_prompt, seed, init_image, mask_image, clip_prompt, guide_image), _) in enumerate(batch):
                ...
                if i2i_noises is not None:  # img2img noise
                    ...
                    # i2i_noises[i] = torch.load("/root/limiao/result/res_universe/noise.pt").to("cuda")
                    print("i2i_noises:", i2i_noises)
