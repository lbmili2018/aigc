    ...
    def __call__(
        ...
        **kwargs,
        ...
        if init_image is None:
            ...
            print("init_latents_img2img_noise:", init_latents.shape, img2img_noise.shape)
        ...
        # if not return_dict:
        return (image, has_nsfw_concept, guided_hints)
...
def main(args):
    ...
    print("args.control_net_models:", args.control_net_models)
    print("args.control_net_preps:", args.control_net_preps)
    print("args.control_net_weights:", args.control_net_weights)
    print("args.control_net_ratios:", args.control_net_ratios)
    ...
    for gen_iter in range(args.n_iter):
        ...
        # バッチ処理の関数
        def process_batch(batch: List[BatchData], highres_fix, highres_1st=False, is_mf_render=False,
                          first_result=None, first_hed=None, last_result=None, last_hed=None):
            ...
            for i, (_, (_, prompt, negative_prompt, seed, init_image, mask_image, clip_prompt, guide_image), _) in enumerate(batch):
                ...
                if init_image is not None:
                    # print("init_image_8888:", init_image.size)
            ...
            if is_mf_render:
                concat_img = PIL.Image.new('RGB', (hed_w * 3, hed_h))
                concat_img.paste(last_result, (0, 0))
                concat_img.paste(init_images, (hed_w, 0))
                concat_img.paste(first_result, (hed_w * 2, 0))

                print("init_images:", init_images.size)
                print("first_result:", first_result.size)
                print("first_hed:", first_hed.size)


                concat_hed = PIL.Image.new('RGB', (hed_w * 3, hed_h), 'black')
                concat_hed.paste(last_hed, (0, 0))
                concat_hed.paste(guide_images, (hed_w, 0))
                concat_hed.paste(first_hed, (hed_w * 2, 0))

                guide_images = concat_hed
                init_images = concat_img

                print("concat_img:", concat_img.size)
                print("concat_hed:", concat_hed.size)

                ## noise
                latent_width = hed_w // 8
                latent_height = hed_h // 8
                i2i_noises = (1, 4, latent_height, latent_width * 3)
                i2i_noises = torch.randn(i2i_noises, dtype=dtype).cuda()

            images, _ , guided_hints = pipe(
                prompts,
                negative_prompts,
                init_images,
                mask_images,
            ...
            if is_mf_render:
                init_images = orig_img
                images = [images[0].crop((hed_w, 0, hed_w * 2, hed_h))]

            # print("guided_hints:", len(guided_hints), guided_hints[0].shape)
        ...
        while args.interactive or prompt_index < len(prompt_list):
            ...
            if seeds is not None:
                ...
                if predefined_seeds is not None:
                    ...
                    print("predefined_seeds:", predefined_seeds)
            ...
            print("seeds:", seeds)
            # hed = HEDdetector.from_pretrained(pretrained_model_or_path="lllyasviel/ControlNet")
            # detect_resolution = min(hed_w, hed_h)
            ...
            guide_image = [guide_image.resize((hed_w, hed_h))]
            ...
            b1 = BatchData(
                False,
                BatchDataBase(global_step, prompt, negative_prompt, seeds[0], init_image, mask_image, clip_prompt,
                              guide_image),
                BatchDataExt(
                    width,
                    height,
            ...
            if len(batch_data) == args.batch_size:
                print("first frame")
                inputss, prev_imagess = process_batch(batch_data, highres_fix)
                # input = inputss[0]
                prev_image = prev_imagess[0]
                print("prev_image111:", prev_image.size)
                first_result = prev_image
                first_hed = guide_image[0]
                last_result = prev_image
                last_hed = guide_image[0]
                batch_data.clear()

            for seed in seeds[1:]:  # images_per_promptの数だけ
                # 同一イメージを使うとき、本当はlatentに変換しておくと無駄がないが面倒なのでとりあえず毎回処理する
                if init_images is not None:
                    init_image = init_images[global_step+1 % len(init_images)]
                    print("init_images_len:", len(init_images))  ## 271
                    print("BatchData_init_image_:", init_image.size)  ## (256, 256)
                ...
                if mask_images is not None:
                    mask_image = mask_images[global_step+1 % len(mask_images)]

                if guide_images is not None:
                    if control_nets:  # 複数件の場合あり
                        c = len(control_nets)
                        p = global_step+1 % (len(guide_images) // c)
                        guide_image = guide_images[p * c : p * c + c]
                    else:
                        guide_image = guide_images[global_step+1 % len(guide_images)]
                elif args.clip_image_guidance_scale > 0 or args.vgg16_guidance_scale > 0:
                    if prev_image is None:
                        print("Generate 1st image without guide image.")
                    else:
                        print("Use previous image as guide image.")
                        guide_image = prev_image
                guide_image = [guide_image[0].resize((hed_w, hed_h))]
                ...
                if len(batch_data) == args.batch_size:
                    ...
                    # hed_image = hed_image.resize((hed_w, hed_h))
                    # ################## hed ####################

                    orig_imgs, res = process_batch(batch_data, highres_fix, is_mf_render=True,
                                                             first_result=first_result, first_hed=first_hed,
                                                             last_result=last_result, last_hed=last_hed)


                    orig_img = orig_imgs
                    res = res[0]

                    res = res.crop((hed_w, 0, hed_w * 2, hed_h))
                    # print("result_new:", result.size)
                    last_result = res
                    last_hed = guide_image[0]

                    print("last_result:", res.size)
                    print("last_hed:", last_hed.size)
